...
| R6 | `sb_secret_…` API key + DB password exposed in a Sign2Sign working session 2026-05-29 | **High** | ✅ **Closed 2026-06-02.** User rotated both via dashboard. Both edge functions (`validate-code`, `delete-admin-account`) redeployed defensively and smoke-tested end-to-end (create user → sign in → delete via function → verify 404; validate-code reachable). Repo scan confirmed `.env.local` does not hold either secret (only the public anon key + Supabase URL + Google IDs), so no local-file update needed. |
...
- [x] ~~R6: rotate exposed credentials~~ — rotated + redeployed + smoke-tested 2026-06-02.
