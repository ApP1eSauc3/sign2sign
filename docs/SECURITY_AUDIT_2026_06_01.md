...
| R6 | `sb_secret_…` API key + DB password exposed in a Sign2Sign working session 2026-05-29 (per `CODEBASE_STATUS.md` Open Decisions) | **High** | **Open.** Rotate both in dashboard → Settings → Database / API Keys. After rotation, redeploy both edge functions so they pick up the new service-role key (`supabase functions deploy validate-code && supabase functions deploy delete-admin-account`), and update `.env.local` for local dev. No external systems depend on Sign2Sign's DB. |
