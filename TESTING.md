...
- **Coverage:** `src/stores/__tests__/useDriverSession.test.ts` and `src/services/__tests__/GoogleSheetsService.test.ts`. (44 tests as of 2026-06-10 — counts drift; `npm test` output is the truth, per the project guide → Documentation Integrity Rules.) These are the canonical, *accurate* examples — prefer them over the illustrative snippets further down, which predate the implementation and may not match current signatures.
